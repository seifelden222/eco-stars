<header class="bg-white/70 backdrop-blur-md border-b border-slate-100 sticky top-0 z-20 px-4 sm:px-8 py-4">
    <div class="max-w-7xl mx-auto flex items-center justify-between">
        ...
        <div class="flex items-center gap-4 flex-1">
            ...
            <button @click="open = !open"
                class="lg:hidden size-11 bg-primary rounded-xl flex items-center justify-center text-white shadow-lg shadow-primary/20 transition-transform active:scale-90">
        ...
        <div class="flex items-center gap-3 sm:gap-6">
            ...
            <div class="bg-yellow-50 px-3 sm:px-5 py-2 rounded-full flex items-center gap-2 border border-yellow-100 shadow-sm hover:scale-105 transition-transform cursor-help"
                title="رصيد نجومك">
                <span
                    class="material-symbols-outlined text-yellow-500 fill-current animate-pulse text-xl sm:text-2xl">stars</span>
            ...
            <div class="relative" x-data="{ dropdownOpen: false }">
                <button @click="dropdownOpen = !dropdownOpen" @click.away="dropdownOpen = false"
                    ...
                    <div class="text-right hidden sm:block">
                        <p
                            class="text-sm font-black text-slate-800 leading-none group-hover:text-primary transition-colors">
                            {{ Auth::user()->name }}
                        </p>
                        <p class="text-[10px] text-slate-400 font-bold uppercase tracking-wider mt-1">البطل المكتشف</p>
                    </div>
                    <div
                        class="size-11 sm:size-12 rounded-2xl bg-slate-100 border-2 border-white shadow-sm overflow-hidden flex items-center justify-center group-hover:border-primary transition-all">
                ...
                <div x-show="dropdownOpen" x-transition:enter="transition ease-out duration-200"
                    ...
                    <a href="{{ route('profile') }}"
                        class="flex items-center gap-3 px-4 py-3 text-sm font-bold text-slate-600 hover:bg-green-50 hover:text-primary transition-colors">
    ...
    <div x-show="open" x-transition:enter="transition ease-out duration-200"
        ...
        class="lg:hidden mt-4 pb-4 space-y-2" style="display: none;">
        <div class="pt-2 pb-3 space-y-1">
            <a href="{{ route('dashboard') }}"
                class="block px-4 py-3 rounded-xl font-bold {{ request()->routeIs('dashboard') ? 'bg-primary text-white shadow-lg shadow-primary/20' : 'text-slate-600 bg-slate-50' }}">
                الرئيسية
            </a>
