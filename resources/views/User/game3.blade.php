...
@section('title','لعبة النجوم - Eco Stars')

@section('content')
<!-- Game 3 content -->
<div class="galaxy-bg h-screen w-screen relative overflow-hidden select-none">
  ...
  <style type="text/tailwindcss">
    body { font-family: 'Cairo', sans-serif; background-color: #020617; overflow: hidden; margin: 0; touch-action: none; }
    .galaxy-bg { background: radial-gradient(circle at 20% 30%, rgba(79, 70, 229, 0.2) 0%, transparent 40%), radial-gradient(circle at 80% 70%, rgba(147, 51, 234, 0.2) 0%, transparent 40%), radial-gradient(circle at 50% 50%, rgba(16, 185, 129, 0.1) 0%, transparent 50%), #020617; }
    .nebula { filter: blur(80px); opacity: 0.4; pointer-events: none; }
    .star-item { filter: drop-shadow(0 0 15px rgba(250, 204, 21, 0.6)); transition: transform 0.15s ease-out; }
    .glass-ui { background: rgba(255, 255, 255, 0.1); backdrop-filter: blur(12px); border: 1px solid rgba(255, 255, 255, 0.2); box-shadow: 0 8px 32px 0 rgba(0, 0, 0, 0.37); }
    .planet { box-shadow: inset -20px -20px 50px rgba(0,0,0,0.5); }
    .energy-fill { box-shadow: 0 0 15px rgba(34, 197, 94, 0.8); transition: width 0.4s ease; }
    #gameOverScreen { background: rgba(2,6,23,0.92); backdrop-filter: blur(8px); }
  </style>
  ...
  <div class="absolute inset-0" id="starsContainer"></div>
